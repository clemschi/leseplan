...
function stoebernOeffnen() {
  ...
  const schrittSammlung = () => {
    ...
    if (!gemerkt.length) {
      inhalt.innerHTML = `
        ...
        <div class="sto-fuss">${zeiger < stapel.length
        ? '<button class="btn btn-block" data-weiter>Weiter stöbern · ' + (stapel.length - zeiger) + ' offen</button>'
        : '<button class="btn btn-block" data-andereszeug>' + ICON.books + ' Andere Sammlung laden</button>'}</div>`;
      /* Ist der Stapel durch, führt „Weiter stöbern“ ins Leere – dann steht
         hier der Weg, der wirklich weiterhilft. */
      const w = $('[data-weiter]', inhalt);
      if (w) w.onclick = () => { schritt = 'wischen'; malen(); };
      const a = $('[data-andereszeug]', inhalt);
      if (a) a.onclick = () => { schritt = 'laden'; malen(); };
      return;
    }
    const nachThema = [];
  ...
  const fussZurueck = () => {
    /* Erst abräumen: hat ein Schritt beim Malen an einen anderen abgegeben,
       steht sein Knopf schon da, und ein zweiter käme obendrauf. */
    $$('.sto-zurueck', inhalt).forEach(x => x.remove());
  ...
  const malen = () => {
    /* Erst den Schritt zurechtrücken, dann malen. Sonst gäbe ein Schritt
       mitten im Malen an den nächsten ab, und was danach kommt – der
       Zurück-Knopf im Fuss – landete zweimal in derselben Fläche. */
    if (schritt === 'wischen' && zeiger >= stapel.length) schritt = 'sammlung';
    if ((schritt === 'wischen' || schritt === 'sammlung' || schritt === 'bloecke') && !stapel.length) {
      schritt = werke.length ? 'themen' : 'laden';
    }
